...
def KerasModel(Dict):
    ...
    model = Sequential()
    model.add(Dense(Dict['L1Neurons'], input_dim=Dict['input_dim'], activation=Dict['activation'], use_bias=False))
    ...
    model.add(Dense(Dict['output'], activation=Dict['activation'], use_bias=False))
...
def AnswerQuestions(numberofQuestions, DirichletProbs):
    test = np.random.choice([-1,0,1], numberofQuestions, p=DirichletProbs)
    return test
...
def syntheticdata(choice):
    ...
    if choice == 1:
        for x in range(Subjects):
            ...
            [DirichletProbabilities] = np.random.dirichlet((a, (a ** 2 + b ** 2), b), size=1).round(10)
        ...
        ListsofQuestions = [[random.randint(-1,1) for j in range(Questions)] for i in range(Subjects)]
...
for trainIndex, testIndex in kfold.split(X):
    ...
    print("weights first layer \n" + str(first_layer_weights))
    # print("biases first layer \n" + str(first_layer_biases))

    second_layer_weights = model.layers[1].get_weights()[0]
    # second_layer_biases = model.layers[1].get_weights()[1]
    print("weights second layer \n" + str(second_layer_weights))
    # print("biases second layer \n" + str(second_layer_biases))
...
#     y_train = train[['Sum_of_Scores']]
#     X_test = test[test.columns[:-1]]
#     y_test = test[['Sum_of_Scores']]
...
# From here: https://snakify.org/en/lessons/two_dimensional_lists_arrays/
